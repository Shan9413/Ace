...
async def account_login(bot: Client, m: Message):
    ...
    editable4 = await m.reply_text(
        "Now send the **Thumb url**\nEg : ```https://telegra.ph/file/8fb6fb525a6ce2e475b41.jpg```\n\nor Send **no**"
    )
    ...
    try:
        for i in range(arg, len(links)):
            ...
            try:
                ...
                name = f'{str(count).zfill(3)}) REXODAS {name1} {res}(@Rexo_Talk_bot)'
            except Exception:
                res = "NA"
            ...
            try:
                ...
                if cmd == "pdf" or ".pdf" in url or ".pdf" in name:
                    try:
                        ...
                        time.sleep(1)
                        reply = await m.reply_text(f"Uploading - ```{name}```(@Rexo_Talk_bot)")
                        time.sleep(1)
